...
const icons = { NextJs, StyledComponents, ReactJs, Javascript, Typescript }
...
const AboutSection = ({ bio, skilss }: props) => {
  ...
  const GetIcon = (icon?: keyof typeof icons) => {
    const value = Object.keys(icons).find(
      (name) => name === icon?.trim().toLowerCase(),
    )
      ...
      <div className="about-inner">
        ...
        <StyledStack>
          ...
          <div className="charts-container">
            {skilss &&
              skilss.map((skill: { name?:  keyof typeof icons }, index) => {
                const icons = GetIcon(skill.name)
                return (
